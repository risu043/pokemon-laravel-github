...
export const getAllFavoritePokemon = async (
    url: string
): Promise<FavoritePokemonResponse> => {
    const response = await fetch(url);
    if (!response.ok) {
        throw new Error("Network response was not ok");
    }
...
export const loadPokemonDetails = async (
    ...
    let pokemonAbilitiesUrl: string[][] = _pokemonData.map((pokemon) => {
        let abilityURL = pokemon.abilities.map(
...
// idからポケモンの詳細データを日本語で取得する関数
export const loadFavoritePokemonDetails = async (
    data: FavoritePokemonResponse[]
): Promise<PokemonProperties[]> => {
    // ポケモン配列から2階層目の詳細データを取得
    let _pokemonDetails = await Promise.all(
        data.map((pokemon) => {
    ...
    // タイプに関する詳細なデータを含むurlを取得
    let pokemonTypesUrl: string[][] = data.map((pokemon) => {
        let typesURL = pokemon.types.map((type) => type.type.url);
        return typesURL;
    });
    ...
    // アビリティに関する詳細なデータを含むurlを取得
    let pokemonAbilitiesUrl: string[][] = data.map((pokemon) => {
        let abilityURL = pokemon.abilities.map(
            (ability) => ability.ability.url
        );
        return abilityURL;
    ...
    // ポケモンの日本語プロパティをまとめる
    const pokemonPropaties: PokemonProperties[] = data.map((pokemon, index) => {
        return {
            id: pokemon.id,
            thumbnail: pokemon.sprites.front_default,
            image: pokemon.sprites.other["official-artwork"].front_default,
